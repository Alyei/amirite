...
export default class QuestionQ extends React.Component {
    ...
    componentWillUnmount() {
        this.props.socketio.socket.emit('leave game', {gameid: this.props.match.params.gameid, username: this.state.username});
        this.props.socketio.socket.off('feedback');
    }
    handleJoinGame() {
        console.log("join game");
        this.props.socketio.socket.emit('join game', {gameid: this.props.match.params.gameid, username: this.state.username});
    }
    ...
    printAnswers(answ) {
        const Answ = answ.map(function (item, i){
            return (
                <AnswerBox key={item[0]} id={item[0]} text={item[1]} onClick={this.AnswClick} />
             );
        }.bind(this)); 
        return(
            <div>
    ...
    render() {
        return(
            <div>
            <QuestionBox id={this.state.questionId} text={this.state.question} pictureId={this.state.pictureId}/>
