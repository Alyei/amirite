import { MessageType, Gamemode, iGeneralHostArguments, iDeterminationHostArguments, iGeneralQuestion, iGeneralPlayerInputError, GameAction, iDeterminationGameData, iDeterminationTip } from "../models/GameModels";
...
export class DeterminationGame implements iGame {
    ...
    public constructor(
        ...
        public Send: (gameId: string, username: string, msgType: MessageType, data: {}) => void,
        public GameEnded: () => void,
        private _gameCoreArguments?: iDeterminationHostArguments
    ) {
        this.GameCore = new DeterminationCore(this.LogInfo, this.LogSilly, this.SendToUser, this.SendGameData, this.LoadQuestions(), this.GeneralArguments.usernames, this._gameCoreArguments);
    }
    ...
    public PerformAction(actionArguments: any): any {
        if ("gameAction" in actionArguments)
            switch (actionArguments.gameAction) {
                ...
                default: {
                    return { message: "action not available for this gamemode", data: actionArguments };
                }
            }
        return { message: "invalid parameter", actionArguments };
    }

    public ProcessUserInput(username: string, msgType: MessageType, data: string): void {
        switch (msgType) {
