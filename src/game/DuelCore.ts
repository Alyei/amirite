...
import {
  ...
  iDuelTipData,
  iDuelScoringData,
  iDuelChooseChoiceReply,
  DuelChoice,
  iDuelChooseCategoryRequest,
  iDuelChooseDifficultyRequest,
  iDuelChooseDifficultyReply,
  iDuelChooseCategoryReply,
  iDuelSetReadyState
...
export class DuelCore {
  ...
  private InformPlayer(
    ...
    const result: boolean = th.Tryhard(
      () => {
        return player.Inform(msgType, data);
      },
      3000, // delay
      3 // tries
    );
  ...
  private SendGameData(): void {
    ...
    for (let player of players) {
      this.InformPlayer(player, MessageType.DuelEndGameData, gameData);
    }
  ...
  private DisqualifyPlayer(player: DuelPlayer): void {
    if (DuelGamePhase.Setup)
      this.players.splice(
        this.players.findIndex(p => p.username == player.username),
        1
      );
    else {
  ...
  private CheckForEnd(): boolean {
    ...
    if (this.gamePhase == DuelGamePhase.Running) {
      ...
      if (
        this.players.find(
          player =>
            [PlayerState.Disqualified, PlayerState.Finished].find(
  ...
  private AskQuestion(): void {
    ...
    if (!this.difficultyReply) this.difficultyReply = { difficulty: -1 };
    if (!this.categoryReply) this.categoryReply = { category: "none" };

    this.questionBases.sort((a, b) => a.questionCounter - b.questionCounter);

    let questionBase: iDuelQuestionBase | undefined = this.questionBases.find(
      q =>
  ...
  private ValidateTip(player: DuelPlayer, opponent: DuelPlayer) {
    ...
    if (
      this.currentQuestion.tip.answerId == "none" ||
      this.currentQuestion.tip.duration >
        this.currentQuestion.question.timeLimit
    ) {
      // too late
      this.currentQuestion.tip.correct = false;
      this.currentQuestion.tip.message = "too slow";
      this.currentQuestion.feedback.scoring[
        player.username
      ].points = Math.floor(-this.gameArguments.pointDeductionWhenTooSlow);
      this.currentQuestion.feedback.scoring[
        opponent.username
      ].points = this.currentQuestion.feedback.scoring[player.username].points;
    } else {
      if (
        this.currentQuestion.tip.answerId == this.currentQuestion.correctAnswer
      ) {
        // correct
        ...
        this.currentQuestion.feedback.scoring[
          ...
          -this.currentQuestion.question.difficulty *
            (this.gameArguments.pointDeductionBase +
              this.currentQuestion.question.timeLimit /
                (this.gameArguments.pointDeductionBase2 +
                  this.currentQuestion.tip.duration))
        );
      }
    }
    player.score += this.currentQuestion.feedback.scoring[
      player.username
    ].points;
    opponent.score += this.currentQuestion.feedback.scoring[
      opponent.username
    ].points;

    this.currentQuestion.feedback.scoring[player.username].score = player.score;
    this.currentQuestion.feedback.scoring[opponent.username].score =
      opponent.score;
      ...
      if (this.currentQuestion.feedback.scoring[p.username])
        ...
        this.LogInfo(
          "stranger things happened: " +
            JSON.stringify(this.players.map(x => x.GetPlayerData()))
        );
      this.InformPlayer(
        p,
  ...
  public ChooseChoice(username: string, choiceChoice: iDuelChooseChoiceReply) {
    ...
    const opponent: DuelPlayer | undefined = this.players.find(
      p => p.username != username
    );
    if (!opponent) {
    ...
    for (let i: number = 0; i < this.questionBases.length; i++) {
      for (
        ...
        if (
          !categories.find(dist => dist == this.questionBases[i].categories[j])
        ) {
    ...
    for (let i: number = 0; i < this.questionBases.length; i++) {
      if (
        !difficulties.find(dist => dist == this.questionBases[i].difficulty)
      ) {
    ...
    this.timers["chooseDifficulty"] = global.setTimeout(() => {
      if (
        this.difficultyChooser &&
        this.difficultyRequest &&
        this.difficultyRequest.difficulties.length > 0
      ) {
        this.ChooseDifficulty(this.difficultyChooser.username, {
          difficulty: this.difficultyRequest.difficulties[
            Math.floor(
              Math.random() * this.difficultyRequest.difficulties.length
            )
          ]
        });
      }
    }, this.gameArguments.choosingTime2);
    this.timers["chooseCategory"] = global.setTimeout(() => {
      if (
        this.categoryChooser &&
        this.categoryRequest &&
        this.categoryRequest.categories.length > 0
      ) {
        this.ChooseCategory(this.categoryChooser.username, {
          category: this.categoryRequest.categories[
            Math.floor(Math.random() * this.categoryRequest.categories.length)
          ]
        });
      }
    }, this.gameArguments.choosingTime2);
  ...
  public ChooseDifficulty(
    username: string,
    choice: iDuelChooseDifficultyReply
  ) {
    ...
    if (
      !this.difficultyRequest.difficulties.find(d => d == choice.difficulty)
    ) {
    ...
    if (this.categoryReply) this.AskQuestion();
  }
  ...
  public ChooseCategory(username: string, choice: iDuelChooseCategoryReply) {
    ...
    if (this.difficultyReply) this.AskQuestion();
  }
